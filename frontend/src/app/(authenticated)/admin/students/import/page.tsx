...
const PENDING_IMPORT_KEY = 'clyvero.student-import.pending-id';

function classTokens(value: string): string[] {
  let normalized = value.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase()
    .replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '');
  const aliases: Array<[RegExp, string]> = [
    [/\b(?:form_?1|premiere_annee|1ere_annee)\b/g, 'level_form1'],
    [/\b(?:form_?2|deuxieme_annee|2eme_annee)\b/g, 'level_form2'],
    [/\b(?:form_?3|troisieme_annee|3eme_annee)\b/g, 'level_form3'],
    [/\b(?:form_?4|quatrieme_annee|4eme_annee)\b/g, 'level_form4'],
    [/\b(?:form_?5|seconde|2nde|cinquieme_annee|5eme_annee)\b/g, 'level_form5'],
    [/\b(?:lower_?sixth|lower_?6th|premiere)\b/g, 'level_lower_sixth'],
    [/\b(?:upper_?sixth|upper_?6th|terminale)\b/g, 'level_upper_sixth'],
    [/\b(?:motor_?mechanics?|mecanique_?automobile)\b/g, 'stream_motor_mechanics'],
    [/\b(?:electrical_?engineering|electricity|genie_?electricite|genie_?electrique)\b/g, 'stream_electricity'],
    [/\b(?:accounting|comptabilite)\b/g, 'stream_accounting'],
    [/\b(?:bespoke_?tailoring|tailoring|couture_?sur_?mesure|couture)\b/g, 'stream_tailoring'],
    [/\b(?:carpentry|menuiserie)\b/g, 'stream_carpentry'],
    [/\b(?:weldering|welding|soudure)\b/g, 'stream_welding'],
    [/\b(?:building_?and_?construction|construction|maconnerie)\b/g, 'stream_construction'],
    [/\b(?:nursing|soins_?infirmiers?)\b/g, 'stream_nursing'],
    [/\b(?:home_?economics|esf)\b/g, 'stream_home_economics'],
  ];
  for (const [pattern, replacement] of aliases) normalized = normalized.replace(pattern, replacement);
  const ignored = new Set(['class', 'classe', 'cycle', 'section', 'secondary', 'secondaire', 'technical', 'technique', 'general']);
  return normalized.split('_').filter((token) => token && !ignored.has(token));
}

function classMatches(source: string, item: { name: string; grade_level?: string | null; speciality?: string | null; speciality_label?: string | null }): boolean {
  const required = classTokens(source);
  if (!required.length) return false;
  const available = new Set(classTokens([item.name, item.grade_level, item.speciality, item.speciality_label].filter(Boolean).join(' ')));
...
export default function OfficialStudentImportPage() {
  ...
  const { data: classes, refetch: refetchClasses, isFetching: classesLoading } = useClasses(yearId ? { academic_year_id: yearId } : {});
  const externalClasses = useMemo(() => preview?.external_classes ?? [], [preview]);
  const completed = preview?.status === 'completed';

  // Student import persistence and class refresh hardening.
  useEffect(() => {
    const recover = async () => {
      setBusy(true);
      try {
        let pendingId = window.sessionStorage.getItem(PENDING_IMPORT_KEY);
        if (!pendingId) {
          const imports = await studentsApi.listOfficialImports();
          pendingId = imports.find((item) => item.status === 'previewed' || item.status === 'processing')?.id ?? null;
        }
        if (!pendingId) return;
        const result = await studentsApi.getOfficialImport(pendingId);
        setPreview(result);
        setClassMapping(result.class_mapping ?? {});
        setDuplicateAction(result.duplicate_action ?? 'skip');
        setStep(3);
        window.sessionStorage.setItem(PENDING_IMPORT_KEY, result.id);
      } catch {
  ...
  useEffect(() => {
    if (preview?.id) window.sessionStorage.setItem(PENDING_IMPORT_KEY, preview.id);
  }, [preview?.id]);

  useEffect(() => {
    if (!preview || completed) return;
    const refresh = () => { void refetchClasses(); };
    window.addEventListener('focus', refresh);
    return () => window.removeEventListener('focus', refresh);
  }, [preview, completed, refetchClasses]);

  useEffect(() => {
    if (!classes?.length || !externalClasses.length) return;
    setClassMapping((current) => {
      const next = { ...current };
      let changed = false;
      for (const sourceClass of externalClasses) {
        const selectedStillExists = classes.some((item) => item.id === next[sourceClass] && item.is_active);
        if (selectedStillExists) continue;
        const matches = classes.filter((item) => item.is_active && classMatches(sourceClass, item));
        const resolved = matches.length === 1 ? matches[0].id : '';
        if ((next[sourceClass] ?? '') !== resolved) { next[sourceClass] = resolved; changed = true; }
      }
  ...
  return <div>
    ...
    {step === 3 && preview && <>
      {completed ? <Card className="mb-6 border-success/30 bg-success-light"><CardHeader><CardTitle>{ui('Import terminé', 'Import completed')}</CardTitle><CardDescription>{ui('Le résultat est conservé pour audit. Aucun paiement n’a été créé.', 'The result is retained for audit. No payment was created.')}</CardDescription></CardHeader><CardContent><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[
        ...
        <Card className="mb-6"><CardHeader><CardTitle>{ui('Mapper les classes', 'Map classes')}</CardTitle><CardDescription>{ui(`Année active : ${preview.academic_year?.title}. Cette étape est encore un aperçu : aucun élève ne sera créé avant la confirmation finale.`, `Active year: ${preview.academic_year?.title}. This is still a preview: no student is created until final confirmation.`)}</CardDescription></CardHeader><CardContent className="space-y-3">
          ...
          {externalClasses.some((name) => !classMapping[name]) && <div className="rounded-card border border-warning/30 bg-warning-light p-4 text-sm text-secondary-700"><p>{ui('La classe requise est absente ?', 'Required class does not exist?')} <Link target="_blank" href={classesBase} className="font-medium text-primary-700 underline">{ui('Créer la classe dans un nouvel onglet', 'Create the class in a new tab')}</Link>. {ui('Revenez ensuite ici : la liste sera actualisée et une classe compatible sera associée automatiquement.', 'Then return here: the list will refresh and a compatible class will be matched automatically.')}</p><Button className="mt-3" variant="outline" onClick={() => void refetchClasses()} isLoading={classesLoading} leftIcon={<RefreshCw className="h-4 w-4" />}>{ui('Actualiser les classes', 'Refresh classes')}</Button></div>}
        </CardContent></Card>

        {externalClasses.length > 0 && externalClasses.every((name) => Boolean(classMapping[name])) && <div className="mb-6 flex flex-col gap-3 rounded-card border border-success/30 bg-success-light p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold text-success-dark">{ui('Toutes les classes sont associées.', 'All classes are mapped.')}</p><p className="text-sm text-secondary-600">{ui('Cliquez maintenant pour créer les élèves et leurs inscriptions dans les classes choisies.', 'Click now to create the students and their enrollments in the selected classes.')}</p></div><Button onClick={confirmImport} isLoading={busy} leftIcon={<FileSpreadsheet className="h-4 w-4" />}>{ui('Confirmer et inscrire les élèves', 'Confirm and enroll students')}</Button></div>}
